...
/** 
 * Test model: CommunicationModel.test.js
...
describe('Controller:Communication', () => { 
	...
	describe('POST /Communication', () => {
        it('Should created new Communication', done => {
            request(sails.hooks.http.app)
            .post(sails.config.blueprints.prefix + '/Communication')
            .send(data)
            .expect(201)
	...
	describe('GET /Communication', () => {
        it('Should getted Communication', done => {
            request(sails.hooks.http.app)
            .get(sails.config.blueprints.prefix + '/Communication')
            .expect(200)
            .expect('Content-Type', /json/)
            .end((err, res) => {
                if (err)
	...
	describe('GET /Communication/:id', () => {
        it('should respond with the requested Communication:id', done => {
            request(sails.hooks.http.app)
            .get(sails.config.blueprints.prefix + '/Communication/' + data.id)
            .expect(200)
            .expect('Content-Type', /json/)
            .end((err, res) => {
                if (err)
    ...
    describe('PUT /Communication/:id', () => {
        it('should respond updated Communication', done => {
            request(sails.hooks.http.app)
            .put(sails.config.blueprints.prefix + '/Communication/' + data.id)
            .send(data)            
            .expect(200)
            .expect('Content-Type', /json/)
            .end((err, res) => {
    ...
    describe('DELETE /Communication/:id', () => {
        it('should respond with 204 on successful removal', done => {
            request(sails.hooks.http.app)
            .delete(sails.config.blueprints.prefix + '/Communication/' + data.id)
            .expect(204)            
            .end((err, res) => {
                if (err)
                    return done(err);
    ...
    //Clear Communication after testing
     after(function() {
